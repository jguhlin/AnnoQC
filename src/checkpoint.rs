...
pub fn run_step<F>(done_path: &Path, resume: bool, name: &str, log_json: bool, f: F) -> Result<(), String>
where
    F: FnOnce() -> Result<(), String>,
{
    if resume && done_path.exists() {
        if log_json {
            log::info!("{}", serde_json::json!({"event":"resume_skip","step":name,"done":done_path.display().to_string()}));
        } else {
            log::info!(
    ...
    if log_json {
        log::info!("{}", serde_json::json!({"event":"step_finish","step":name,"duration_sec":dt.as_secs_f64()}));
    } else {
