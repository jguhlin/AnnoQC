...
pub fn compute_homology_score(s: Option<&DiamondHitStats>) -> f64 {
    if let Some(s) = s {
        ...
        let density = if s.top_len > 0 { (s.top_bitscore / s.top_len as f64) / 5.0 } else { 0.0 };
...
pub fn combine_scores(homology: f64, intrinsic: f64, w_h: f64, w_i: f64) -> ScoreBreakdown {
    let sum = (w_h + w_i).max(1e-6);
    let final_score = (w_h * homology + w_i * intrinsic) / sum;
