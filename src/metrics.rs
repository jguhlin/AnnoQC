...
pub struct IntrinsicMetrics {
    ...
    #[allow(dead_code)]
    pub start_methionine: bool,
    #[allow(dead_code)]
    pub alt_start_pos: Option<usize>,
    #[allow(dead_code)]
...
pub fn compute_intrinsic(seq: &[u8]) -> IntrinsicMetrics {
    ...
    if terminal_stop && internal_stop_count > 0 {
        internal_stop_count -= 1;
    }
    ...
    if !start_methionine {
        for i in 1..k {
            if seq[i] == b'M' {
                alt_start_pos = Some(i);
                break;
            }
        }
    }
    let mut orf_start_score = if start_methionine {
        1.0
    } else if alt_start_pos.is_some() {
        0.8
    } else {
        0.5
    };
    if terminal_stop {
        orf_start_score = (orf_start_score - 0.2f64).max(0.0);
    }
    if internal_stop_count > 0 {
        orf_start_score = (orf_start_score - 0.3f64).max(0.0);
    }
